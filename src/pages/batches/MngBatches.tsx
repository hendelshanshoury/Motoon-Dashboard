...
const MngBatches = () => {
    const createUrl = '/api/admin/batches';
    const updateUrl = '/api/admin/batches';
    ...
    const { submitCreateForm, updateForm } = useCreate({ createUrl, navigateUrl, updateUrl });
    ...
    return (
        <>
            <div className="py-2">
                ...
                {false && <div className="panel mb-5">
                    <Formik
                        enableReinitialize
                        initialValues={{ id: data?.id, start_date: data?.start_date, code: data?.code }}
                        validationSchema={UpdateStartForm}
                        onSubmit={(values) =>
                            isNew ? submitCreateForm({ start_date: values?.start_date, code: values?.code }) : updateForm({ id: data?.id, start_date: values?.start_date, code: values?.code })
                        }
                    >
                        {({ touched, errors }) => (
                            <Form className={`space-y-5 ${loading ? 'pointer-events-none' : ''}`}>
                                <InputGroup className="grid-cols-1 md:grid-cols-2">
                                    <div>
                                        ...
                                        <Field dir="auto" name={`start_date`} type="date" id="start_date" className="form-input" />
                                        {touched.start_date && <ErrMsg name="start_date" />}
                                    </div>
                                    <div>
                                        <label htmlFor="code">كود الدفعة</label>
                                        <Field dir="auto" name={`code`} id="code" className="form-input" />
                ...
                <div className="panel">
                    ...
                    <Formik
                        ...
                        onSubmit={(values) => (isNew ? submitCreateForm({ ...values, start_date: values['sem1_date'] }) : updateForm({ ...values, start_date: values['sem1_date'] }))}
                    >
                        {({ touched, errors }) => (
                            <Form className={`space-y-5 ${loading ? 'pointer-events-none' : ''}`}>
                                ...
                                <InputGroup className="grid-cols-1 md:grid-cols-2">
                                    <div>
                                        ...
                                        <Field dir="auto" name={`sem1_date`} type="date" id="sem1_date" className="form-input" />
                                        {touched.sem1_date && <ErrMsg name="sem1_date" />}
                                    </div>
                                    <div>
                                        <label htmlFor="sem2_date">موعد فصل دراسي ثاني</label>
                                        <Field dir="auto" name={`sem2_date`} type="date" id="sem2_date" className="form-input" />
                                        {touched.sem2_date && <ErrMsg name="sem2_date" />}
                                    </div>
                                    <div>
                                        <label htmlFor="sem3_date">موعد فصل دراسي ثالث</label>
                                        <Field dir="auto" name={`sem3_date`} type="date" id="sem3_date" className="form-input" />
                                        {touched.sem3_date && <ErrMsg name="sem3_date" />}
                                    </div>
                                    <div>
                                        <label htmlFor="sem4_date">موعد فصل دراسي رابع</label>
                                        <Field dir="auto" name={`sem4_date`} type="date" id="sem4_date" className="form-input" />
