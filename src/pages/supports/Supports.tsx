    ...
    const cols = [
        {
            ...
            render: (data: any) => (
                <ActionsRow
                    editFN={() => navigate(`/contacts/${data.id}`)}
        ...
        },
        {
            ...
            titleClassName: '!text-center',
            render: (data: any) => (
                <a target="_blank" href={`/storage/${data?.image}`}>
                    {data?.image !== '/storage/null' ? <img width={50} height={50} className="border" src={`/storage/${data?.image}`} alt="" /> : 'لا يوجد'}
                </a>
            ),
        },
        ...colsData.map((col) => ({ accessor: col.name, hidden: hideCols.includes(col.name), title: col.title })),
        {
            ...
            titleClassName: '!text-center',
            render: (data: any) => (
                <p onClick={() => navigator.clipboard.writeText(data?.message)} title={data?.message}>
                    {data?.message.slice(0, 40)}
                    {data?.message.length > 40 && <span>...</span>}{' '}
                </p>
            ),
        },
    ];
            ...
            <div className="panel ">
                <div className="flex items-center justify-between ltr:ml-auto rtl:mr-auto mb-3">
                    <div className="flex items-center gap-5">
                        ...
                        {/* ADD BUTTUN
                        <AddLink link={'/courses/mng/new'} title="مادة" /> */}
                    </div>
                    <div>
