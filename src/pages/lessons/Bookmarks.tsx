...
import { colsData_bookmarks as colsData } from '../../utils/columns_tables';
...
const url = '/api/admin/lesson/bookmarks';
    ...
    const cols = [
        {
            ...
            render: (data: any) => (
                <div className="flex gap-2">
                    <ActionsRow editFN={() => navigate(`/bookmarks/mng/${data?.lesson_id}`)} deleteFN={() => setModal({ status: true, activeID: data.id })}/>
                </div>
            ),
        },
            ...
            <div className="panel ">
                <div className="flex items-center justify-between ltr:ml-auto rtl:mr-auto mb-3">
                    <div className="flex items-center gap-5">
                        ...
                        {/* ADD BUTTUN */}
                        {cookie.get('user')?.type !== 'org' && <AddLink link={'/bookmarks/mng/new'} title="علامة مرجعية" />}
                    </div>
                    <div>
...
export const AddLink = ({ link, title }: any) => {
    return (
        <Link to={link}>
