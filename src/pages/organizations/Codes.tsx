...
import { colsData_codes as colsData } from '../../utils/columns_tables';
import Cookies from 'universal-cookie';
const cookie = new Cookies();

export const colsExtend = colsData.filter((col) => col.extended).map((cols) => cols.name);
const url = '/api/admin/codes';

let searchTimeout: any = null;
const Codes = () => {
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(setPageTitle('Codes'));
    });
    ...
    const cols = [
        {
            ...
            render: (data: any) => (
                <ActionsRow
                    editFN={() => navigate(`/organizations/codes/mng/${data.id}`)}
    ...
    return (
        <div>
            ...
            <div className="panel ">
                <div className="flex items-center justify-between gap-5 ltr:ml-auto rtl:mr-auto mb-3">
                    <div className="flex items-center gap-5">
                        ...
                        {/* ADD BUTTUN */}
                        {cookie.get('user')?.type !== 'org' && <AddLink link={'/organizations/codes/mng/new'} title="كود تسجيل" />}
                    </div>
                    <div>
