...
import { colsData_courses as colsData } from '../../utils/columns_tables';
import Cookies from 'universal-cookie';
const cookie = new Cookies();
import Tippy from '@tippyjs/react';
import day_name from '../../utils/days';
    ...
    const cols = [
        {
            ...
            render: (data: any) => <ActionsRow editFN={() => navigate(`/courses/mng/${data.id}`)} deleteFN={() => setModal({ status: true, activeID: data.id })} />,
        },
        {
            accessor: 'day',
            title: 'موعد الدرس',
            titleClassName: '!text-center',
            render: (data: any) => day_name(data?.day),
        },
            ...
            <div className="panel">
                <div className="flex items-center justify-between ltr:ml-auto rtl:mr-auto mb-3">
                    <div className="flex items-center gap-5">
                        ...
                        {/* ADD BUTTUN */}
                        {cookie.get('user')?.type !== 'org' && <AddLink link={'/courses/mng/new'} title="مادة" />}
                    </div>
                    <div className="flex justify-center items-center gap-4">
                        <Link to="/sortableCourse">
