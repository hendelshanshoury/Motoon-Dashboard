...
import { colsData_user as colsData, semseters_names } from '../../utils/columns_tables';
...
const Users = () => {
    ...
    // download excel
    const header = ['#', 'الاسم بالانجليزي', 'الاسم بالعربي', 'الجنسية', 'البريد الالكتروني', 'كود المنظمة', 'الدفعة', 'الفصل الحالي'];
        ...
        downloadExcel({
            ...
            tablePayload: {
                ...
                body: data?.data?.map((x) => ({
                    ...
                    name: x?.name,
                    name_ar: x?.name_ar,
                    country: countries.find((country) => country?.value.toLowerCase() == x.country?.toLowerCase())?.label,
                    email: x?.email,
                    code: x?.code,
            ...
            <div className="panel ">
                <div className="flex items-center justify-between ltr:ml-auto rtl:mr-auto mb-3">
                    <div className="flex flex-wrap items-center gap-5">
                        ...
                        <div className="text-right">
                            <input type="text" className="form-input" placeholder="كود المنظمة" value={code} onChange={(e) => setCode(e.target.value)} />
                        </div>
                        ...
                        {/* ADD BUTTUN */}
                        <input type="date" placeholder="تاريخ التسجيل" className="form-input w-fit" value={date1} onChange={(e) => setDate1(e.target.value)} />
                        <button
                        ...
                        {cookie.get('user')?.type !== 'org' && <AddLink title={''} link={'/users/mng/new'} />}
                    </div>
                    {/* <div>
...
const ExportToExcel = ({ filter }) => {
    async function downloadExcel() {
        const { data } = await axios.get('/api/admin/reports/users', { params: filter });
    ...
    return (
        <div>
            <button type="button" className="btn btn-primary" onClick={downloadExcel}>
                ...
                <span className="ps-2 hidden md:block">تصدير</span>
            </button>
        </div>
    );
};
