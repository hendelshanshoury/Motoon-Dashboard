...
const Header = () => {
    ...
    // const navigateUrl = '/';
    const { navigateUrl, loginUrl } = useAuth({
        loginUrl: '/auth/signin',
        navigateUrl: '#',
    });

    const handleSignOut = () => {
        cookiesInstance.remove('user');
        navigate(loginUrl);
    };
                    ...
                    <div className="sm:flex-1 ltr:sm:ml-0 ltr:ml-auto sm:rtl:mr-0 rtl:mr-auto flex items-center space-x-1.5 lg:space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                        ...
                        <div className="dropdown shrink-0 flex">
                            <Dropdown
                                ...
                                button={<img className="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="/logo.png" alt="userProfile" />}
                            >
                                <ul className="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                                    <li>
                                        <div className="flex items-center px-4 py-4">
                                            <div className="truncate">
                                                <span className="text-xs bg-success-light rounded text-success px-1 ltr:ml-2 rtl:ml-2">{cookiesInstance.get('user')?.type === 'org' ? 'جمعية' : 'مسؤول'}</span>
                                                <h4 className="text-xs">{cookiesInstance.get('user')?.name}</h4>
                                                <button type="button" className="text-black/60 hover:text-primary dark:text-dark-light/60 dark:hover:text-white">
                                                    {cookiesInstance.get('user')?.email}
                                                </button>
                                            </div>
                                        </div>
                                    </li>
                                    ...
                                    <li className="border-t border-white-light dark:border-white-light/10">
                                        <Link to="/auth/signin" onClick={handleSignOut} className="text-danger !py-3">
